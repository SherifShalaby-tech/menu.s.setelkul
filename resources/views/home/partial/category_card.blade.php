<a href="{{ action('ProductController@getProductListByCategory', $category->id) }}"
    ...
    style="background-image: url('{{ !empty($category->getFirstMediaUrl('product_class')) ? images_asset($category->getFirstMediaUrl('product_class')) : images_asset() }}')">
    <div class="flex absolute md:-bottom-16 xs:-bottom-10 w-full">
        <div class="lg:h-10 md:h-8 xs:h-6 md:w-48 xs:w-40 bg-darkblue mx-auto text-center rounded-3xl category">
